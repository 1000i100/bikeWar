...
var MyIA = function() {
	this._turnNum = 1;
	WorkerIA.call(this);
	this._movingTruck = new Array();
	this._targetStations = new Array();
};
MyIA.__name__ = true;
MyIA.main = function() {
...
MyIA.prototype = $extend(WorkerIA.prototype,{
	getOrders: function(context) {
		...
		var result = new Array();
		var _g = 0;
		var _g1 = this._context.trucks;
		while(_g < _g1.length) {
			var truck = _g1[_g];
			++_g;
			if(truck.owner.id == this.id) {
				if(Lambda.has(this._movingTruck,truck.id)) {
					if(truck.currentStation != null) {
						HxOverrides.remove(this._movingTruck,truck.id);
						HxOverrides.remove(this._targetStations,truck.currentStation.id);
						if(this.canLoadEnough(truck,truck.currentStation) && this.getNumberOfBikesToLoad(truck,truck.currentStation) > 0) {
							var numberOfBikes = this.getNumberOfBikesToLoad(truck,truck.currentStation);
							result.push(new com.tamina.bikewar.data.LoadingOrder(truck.id,truck.currentStation.id,numberOfBikes));
						} else if(this.canUnloadEnough(truck,truck.currentStation) && this.getNumberOfBikesToUnload(truck,truck.currentStation) > 0) {
							var numberOfBikes1 = this.getNumberOfBikesToUnload(truck,truck.currentStation);
							result.push(new com.tamina.bikewar.data.UnLoadingOrder(truck.id,truck.currentStation.id,numberOfBikes1));
						} else if(com.tamina.bikewar.game.GameUtils.getBikeStationTrend(truck.currentStation,this._context.currentTime) != -1 && truck.bikeNum < com.tamina.bikewar.game.Game.TRUCK_NUM_SLOT && truck.currentStation.bikeNum > truck.currentStation.slotNum / 4) result.push(new com.tamina.bikewar.data.LoadingOrder(truck.id,truck.currentStation.id,1)); else if(truck.bikeNum > 0 && truck.currentStation.bikeNum < truck.currentStation.slotNum * 3 / 4) result.push(new com.tamina.bikewar.data.UnLoadingOrder(truck.id,truck.currentStation.id,1)); else console.log("No order");
					}
				} else {
					var nextStation = this.getNextStation(truck);
					if(nextStation != null && nextStation.id != truck.currentStation.id) {
						this._targetStations.push(nextStation.id);
						this._movingTruck.push(truck.id);
						result.push(new com.tamina.bikewar.data.MoveOrder(truck.id,nextStation.id));
					}
				}
			}
		}
	...
	,getNextStation: function(truck) {
		var _g = this;
		var stationsArray = this._context.stations.slice();
		HxOverrides.remove(stationsArray,truck.currentStation);
		stationsArray.sort(function(a,b) {
			var aScore = _g.getStationScore(a,truck);
			var bScore = _g.getStationScore(b,truck);
			if(aScore == bScore) return 0; else if(aScore < bScore) return -1; else return 1;
		});
		var _g1 = 0;
		while(_g1 < stationsArray.length) {
			var station = stationsArray[_g1];
			++_g1;
			if(this.validateStation(station,truck)) return station;
		}
		return stationsArray[Math.round(Math.random() * stationsArray.length)];
	}
	,getStationScore: function(station,truck) {
		var stationScore = com.tamina.bikewar.game.GameUtils.getTravelDuration(truck.currentStation,station,this._context);
		if(this.canSteal(station,truck)) stationScore = stationScore / 5; else if(this.mustProtect(station,truck)) stationScore = stationScore / 2;
		return stationScore;
	}
	,canSteal: function(station,truck) {
		return station.owner != null && station.owner.id != this.id && com.tamina.bikewar.game.GameUtils.hasStationEnoughBike(station) && this.validateStation(station,truck);
	}
	,mustProtect: function(station,truck) {
		return station.owner != null && station.owner.id == this.id && !com.tamina.bikewar.game.GameUtils.hasStationEnoughBike(station) && this.validateStation(station,truck);
	}
	,validateStation: function(station,truck) {
		var isValidStation = false;
		if(this.canUnloadEnough(truck,station) || this.canLoadEnough(truck,station)) {
			isValidStation = true;
			if(Lambda.has(this._targetStations,station.id)) isValidStation = false;
		}
		return isValidStation;
	}
	,canLoadEnough: function(truck,station) {
		var stationBikeNumAfterMove = station.bikeNum + this.getStationTrendAfterMove(station,truck);
		var truckSpace = com.tamina.bikewar.game.Game.TRUCK_NUM_SLOT - truck.bikeNum;
		return truckSpace > 0 && station.bikeNum > station.slotNum / 4 && stationBikeNumAfterMove - truckSpace <= station.slotNum / 3 * 4;
	}
	,canUnloadEnough: function(truck,station) {
		var stationBikeNumAfterMove = station.bikeNum + this.getStationTrendAfterMove(station,truck);
		return truck.bikeNum > 0 && station.bikeNum < station.slotNum * 3 / 4 && stationBikeNumAfterMove + truck.bikeNum >= station.slotNum / 4;
	}
	,getStationBikeDiff: function(station) {
		return Math.round(station.slotNum / 2) - station.bikeNum;
	}
	,getStationTrendAfterMove: function(station,truck) {
		var duration = com.tamina.bikewar.game.GameUtils.getTravelDuration(truck.currentStation,station,this._context);
		var time = this._context.currentTime;
		var currentIndex = time.getHours() * 4 + Math.floor(time.getMinutes() * 4 / 60);
		return (station.profile[currentIndex + duration + 5] - station.profile[currentIndex]) * 2;
	}
	,getNumberOfBikesToLoad: function(truck,station) {
		var numberOfBikesNeeded = station.bikeNum - Math.round(station.slotNum / 2);
		if(numberOfBikesNeeded < 0 || com.tamina.bikewar.game.Game.TRUCK_NUM_SLOT - truck.bikeNum >= numberOfBikesNeeded) return numberOfBikesNeeded; else return com.tamina.bikewar.game.Game.TRUCK_NUM_SLOT - truck.bikeNum;
	}
	,getNumberOfBikesToUnload: function(truck,station) {
		var numberOfBikesNeeded = Math.round(station.slotNum / 2) - station.bikeNum;
		if(numberOfBikesNeeded < 0 || truck.bikeNum >= numberOfBikesNeeded) return numberOfBikesNeeded; else return truck.bikeNum;
	}
});
var Std = function() { };
Std.__name__ = true;
