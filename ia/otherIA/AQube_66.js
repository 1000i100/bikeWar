...
Trucks.prototype.mines = function() {
    var _this = this;
    return _.filter(_this.context.trucks, function(truck) {
...
Trucks.prototype.whenAtDestination = function(result, truck, state) {
    ...
    if (!node) {
        NOT_FOUND_COUNTER++;
        console.log('NODE NOT FOUND ! (' + NOT_FOUND_COUNTER + ')');
        return;
    }

    var delta = node.getBikeToMatchDelta(truck, node.station.bikeNum);
    ...
    if(delta < 0) {
        ...
        if ((nextTrend === Trend.DECREASE || nextTrend === Trend.STABLE) && truck.bikeNum > 0) {
            result.push(new UnLoadingOrder(truck.id, truck.currentStation.id, 1));
            truck.currentStation.bikeNum += 1;
            truck.bikeNum -= 1;
        } else if ((nextTrend === Trend.INCREASE || nextTrend === Trend.STABLE) && truck.bikeNum < Game.TRUCK_NUM_SLOT) {
...
Trucks.prototype.onStartOrLeavingStation = function(result, truck, state) {
    ...
    if (destination !== undefined && destination !== null) {
        ...
        _targetOfTruck[truck.id] = destination;
        result.push(new MoveOrder(truck.id, destination.id));
    } else {
        console.log("no destination found");
    }
}

Trucks.prototype.onTheRoadMoving = function(result, truck) {

}

Trucks.prototype.atDestination = function(truck) {
    var _this = this;
    truck.currentStation.taken = false;
    delete _targetOfTruck[truck.id];
    return _this.movingTruckId.indexOf(truck.id) > -1;
}

Trucks.prototype.getNextDestination = function(truck, state) {
    return state.getBestNodeStation();
}

var Stations = function(context, id) {
    this.context = context;
    this.id = id;
};

Stations.prototype.getRandomStation = function() {
    var _this = this;
    return _this.context.stations[Math.round(Math.random() * _this.context.stations.length)];
}

Stations.prototype.all = function() {
    var _this = this;
    return _.filter(_this.context.stations, function(station) {
        return !station.taken;
    });
}

var NodeStation = function(id, truck, station) {
    this.id = id;
    this.truck = truck;
    this.station = station;
    this.distance = 0;
    this.capacityAvg = station.bikeNum;
...
NodeStation.prototype.getBikeToMatchDelta = function(truck, capacity) {
    ...
    var offset = this.station.bikeNum - ideal;
    ...
    var newTruckCapacity = truck.bikeNum + offset;
    ...
    var newStationCapacity = this.station.bikeNum - offset;
    if (newStationCapacity > this.station.slotNum) {
        offset += (newStationCapacity - this.station.slotNum);
    } else if (newStationCapacity < 0) {
        offset -= (newStationCapacity * -1);
    }

    //console.log('ideal: ' + ideal + ' (' + this.station.bikeNum + ' / ' + this.station.slotNum + ') ; offset: ' + offset + ' ; newCapacity: ' + newTruckCapacity);

    return offset;
}
NodeStation.prototype.computeDistance = function(context) {
    this.distance = GameUtils.getTravelDuration(this.truck.currentStation, this.station, context);
}
NodeStation.prototype.computeCapacityAtDistance = function(context) {
    var _this = this;

    function changeCapacity(actual, offset) {
        var capacity = actual + offset;
        if (capacity < 0) {
            capacity = 0;
        } else if (capacity > _this.station.bikeSlot) {
            capacity = _this.station.bikeSlot;
        }
        return capacity;
    }

    function pushDecrease() {
        _this.capacityMax = changeCapacity(_this.capacityMax, 0);
        _this.capacityMin = changeCapacity(_this.capacityMin, -3);
        _this.capacityAvg = changeCapacity(_this.capacityAvg, -1.5);
    }
    function pushIncrease() {
        _this.capacityMax = changeCapacity(_this.capacityMax, 3);
        _this.capacityMin = changeCapacity(_this.capacityMin, 0);
        _this.capacityAvg = changeCapacity(_this.capacityAvg, 1.5);
    }
    function pushStable() {
        _this.capacityMax = changeCapacity(_this.capacityMax, 1);
        _this.capacityMin = changeCapacity(_this.capacityMin, -1);
        _this.capacityAvg = changeCapacity(_this.capacityAvg, 0);
    }

    for (var futurTurn = _turnNum; futurTurn <= _this.distance; futurTurn++) {
        var time = _this.getTimeForTurn(context, futurTurn);
        var trend = _this.getTrend(_this.station, time);
...
NodeStation.prototype.computeScore = function(truck) {
    var _this = this;

    function overCapacity(capacity) {
        return capacity >= _this.station.slotNum / 4*3;
    }
    function underCapacity(capacity) {
        return capacity <= _this.station.slotNum / 4;
    }

    function truckEmpty() {
        return truck.bikeNum == 0;
    }
    function truckFull() {
        return truck.bikeNum == Game.TRUCK_NUM_SLOT;
    }
    ...
    if (underCapacity(_this.capacityAvg) && truckEmpty()) {
        _this.score -= 1000;
    }
    if (underCapacity(_this.capacityAvg) && truckFull()) {
        _this.score *= 2;
    }
    if (overCapacity(_this.capacityAvg) && truckEmpty()) {
        _this.score *= 2;
    }

    if (!overCapacity(_this.capacityAvg) && !underCapacity(_this.capacityAvg) && _this.station.owner && _this.station.owner.id != _this.id) {
        console.log('IS GOOD FOR OTHER GUY');
        _this.score += 250;
        _this.score *= 2;
    }
...
var State = function(context, id) {
    this.context = context;
    this.id = id;
    this.stations = new Stations(context, id);
    this.nodes = [];
};
State.prototype.analyze = function(truck) {
    var _this = this;

    var stations = _this.stations.all();
    _.forEach(stations, function(station) {
        var node = new NodeStation(_this.id, truck, station);
        node.computeDistance(_this.context);
        _this.nodes.push(node);
    });

    _.forEach(_this.nodes, function(node) {
        node.computeCapacityAtDistance(_this.context);
        node.computeScore(truck);
    });

    _this.nodes = _.sortBy(_this.nodes, 'score');
    _this.nodes.reverse();

}
State.prototype.getBestNodeStation = function() {
    return this.nodes[0].station;
};
State.prototype.getNextNodeStation = function() {
    return this.nodes[1].station;
};
State.prototype.getNodeOfStation = function(station) {
...
var getOrders = function (context) {
    ...
    _.forEach(myTrucks, function(truck) {
        if (_targetOfTruck[truck.id]) {
            _targetOfTruck[truck.id].taken = true;
        }
    });
    ...
    _.forEach(myTrucks, function(truck) {
        if (truck.currentStation == null || truck.currentStation == undefined) {
            ...
            if(trucks.atDestination(truck)) {
                ...
                trucks.onStartOrLeavingStation(result, truck, state);
            }
        }
    });
