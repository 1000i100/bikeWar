			...
			if(truck.owner.id == this.id && truck.currentStation != null) {
				if(Lambda.has(this._movingTruckId,truck.id)) {
					...
					if(truck.bikeNum >= 5 && few.length > 0) {
						...
						result.push(new com.tamina.bikewar.data.MoveOrder(truck.id,few[Math.round(Math.random() * few.length)].id));
					} else if(truck.bikeNum < 5 && much.length > 0) {
						this._movingTruckId.push(truck.id);
						result.push(new com.tamina.bikewar.data.MoveOrder(truck.id,much[Math.round(Math.random() * much.length)].id));
					} else if(sickBadStation.length > 0) {
						this._movingTruckId.push(truck.id);
						result.push(new com.tamina.bikewar.data.MoveOrder(truck.id,sickBadStation[Math.round(Math.random() * sickBadStation.length)].id));
					}
				}
			} else {
			}
