...
NodesAnalyzer.prototype.generate = function() {
    ...
    _.forEach(_this.context.stations, function(station) {
        if (!_.contains(next, station.id) && !_.contains(previous, station.id)) {
            treeNodes.push(new TreeNode(_this.id, _this.context, _this.start, station, _this.truck, station));
        }
    });
...
TreeNode.prototype.compute = function() {
    ...
    _this.score = Math.round(_this.score * (100.0 - distanceTakenIntoAccount * penalty) / 100.0);
}
