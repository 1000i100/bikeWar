		...
		while(_g1 < _g) {
			...
			if(truck.owner.id == this.id && truck.currentStation != null) {
				if(Lambda.has(this._movingTruckId,truck.id)) {
					...
					if(station.bikeNum < station.slotNum / 4) {
						if(truck.bikeNum > 0) result.push(new com.tamina.bikewar.data.UnLoadingOrder(truck.id,station.id,1)); else if(station.bikeNum > 0) result.push(new com.tamina.bikewar.data.LoadingOrder(truck.id,station.id,1));
					} else if(station.bikeNum > station.slotNum / 4 * 3) {
						if(truck.bikeNum < com.tamina.bikewar.game.Game.TRUCK_NUM_SLOT) result.push(new com.tamina.bikewar.data.LoadingOrder(truck.id,station.id,1)); else if(station.bikeNum < station.slotNum) result.push(new com.tamina.bikewar.data.UnLoadingOrder(truck.id,station.id,1));
					} else if(com.tamina.bikewar.game.GameUtils.hasStationEnoughBike(station)) {
						if(truck.bikeNum < com.tamina.bikewar.game.Game.TRUCK_NUM_SLOT) result.push(new com.tamina.bikewar.data.LoadingOrder(truck.id,station.id,1)); else result.push(new com.tamina.bikewar.data.UnLoadingOrder(truck.id,station.id,1));
					}
				} else {
					...
					var station1 = null;
					var _g3 = 0;
					var _g2 = context.stations.length;
					while(_g3 < _g2) {
						var j = _g3++;
						var stationTmp = context.stations[j];
						if(stationTmp.id != truck.currentStation.id && (stationTmp.owner == null || stationTmp.owner.id != this.id)) {
							var dist = com.tamina.bikewar.game.GameUtils.getDistanceBetween(truck.currentStation.position,stationTmp.position);
							if(min == -1. && stationTmp != this.truckOneDestination && stationTmp.owner != null && stationTmp.owner.id != this.id) {
								min = dist;
								station1 = stationTmp;
							} else if(min > dist && stationTmp != this.truckOneDestination && stationTmp.owner != null && stationTmp.owner.id != this.id) {
								min = dist;
								station1 = stationTmp;
							}
						}
					}
					if(this.truckOneDestination == null) this.truckOneDestination = station1;
					result.push(new com.tamina.bikewar.data.MoveOrder(truck.id,station1.id));
				}
			} else {
			}
		}
