...
var lePlusProche = function (context,x,y){
    var j = 0;
    difx = (context.stations[j].position.x - x);
    dify = (context.stations[j].position.y - y);
    var distmin = Math.sqrt(difx*difx + dify*dify);
    var idSta = 0;
    for (j = 1; j < context.stations.length; j++){ // context.stations.length => nombre de stations max, trop long pour un tour
        difx = (context.stations[j].position.x - x);
        dify = (context.stations[j].position.y - y);
...
var lesPlusProche = function (context,x,y,rayon_max){
    var idSta = new Array;
    for (var j = 0; j < context.stations.length; j++){ // context.stations.length => nombre de stations max, trop long pour un tour
        difx = (context.stations[j].position.x - x);
        dify = (context.stations[j].position.y - y);
        dist = Math.sqrt(difx*difx + dify*dify);
        if ( dist < rayon_max) {
...
var getOrders = function (context) {
    ...
    var _g1 = 0;
    var _g = context.trucks.length; // nombre de camion (4 par défaut)
// context.trucks[0].id // identifiant de notre premier camion
// context.trucks[1].id // identifiant de notre deuxieme camion
debugMessage += "Nbr Station" + context.stations.length + "\n"; // nombre de station (80 par défaut)
// debugMessage+="mon message\n"; // affichage un message de debugage dans la console
// context.trucks[0].position.x; // abscisse de notre premier camion 


// CALCUL DE LA POSITION MOYENNE
var moyx=0;
var moyy=0;
for(var j=0; j<context.stations.length; j++){
    //debugMessage += "TOTO= " + context.stations[j].position.x + "\n"; 
    moyx = moyx + context.stations[j].position.x;
    moyy = moyy + context.stations[j].position.y;
}

moyx /= context.stations.length;
moyy /= context.stations.length;
    debugMessage += "========================================================TOTAL POSITION= " + moyx + "," + moyy + "\n";

// RECHERCHE DE LA STATION LA PLUS PROCHE DE LA MOYENNE
var indiceStationPlusProcheCentre = lePlusProche(context,moyx,moyy);
    debugMessage += "========================================================INDICE POSITION= " + indiceStationPlusProcheCentre + "\n";

// RECHERCHE DES STATIONS A L'INTERIEUR DU PERIMETRE
var indicesStationPlusProcheCentre = lesPlusProche(context,moyx,moyy,100);
    debugMessage += "========================================================INDICE POSITION= " + indicesStationPlusProcheCentre + "\n";



    while(_g1 < _g) {// pour chaque camion du jeu
        var i = _g1++;
        var truck = context.trucks[i];

        if(truck.owner.id == this.id && truck.currentStation != null) { // Si c'est l'un de mes camion et qu'il est sur une station
            if( i == 0)
                debugMessage += "HYPOOOOOOOOOOOOOOOOOOOOOOOOO: nbdevelo dans le camion 1 =" + truck.bikeNum + "\n";
            

            if(this._movingTruckId.indexOf(truck.id) > -1) { // si le camion était en déplacement 
                // ORDRE DE GESTION DES VELOS
                
                HxOverrides.remove(this._movingTruckId,truck.id);  // suppression du camion de la liste des camions en déplacement / FIN DU DEPLACEMENT
                if( i == 0){
                    if(truck.currentStation.id == context.stations[0].id){ // si on est sur la premiere station alors 
                        if(truck.bikeNum < Game.TRUCK_NUM_SLOT && truck.currentStation.bikeNum > 0){ // si le camion n'est pas plein et qu'au moins un velo est dans la station 
                            debugMessage += "HYPOOOOOOOOOOOOOOOOOOOOOOOOO: on va charger un velo \n";
                            result.push(new LoadingOrder(truck.id,truck.currentStation.id,1)); // on charge un vélo dans le camion
                        }
                    }else
                        if(truck.bikeNum > 0 && truck.currentStation.slotNum > 0){ // si le camion contient au moins un vélo et qu'au moins un slolt de la station est libre 
                            debugMessage += "HYPOOOOOOOOOOOOOOOOOOOOOOOOO: on va decharger un velo \n";
                            result.push(new UnLoadingOrder(truck.id,truck.currentStation.id,1)); // on dechage un vélo dans le camion
                        }
                    /*else if(GameUtils.hasStationEnoughBike(truck.currentStation)) {
                        if(truck.bikeNum < Game.TRUCK_NUM_SLOT) // si le camion n'est pas plein
                            result.push(new LoadingOrder(truck.id,truck.currentStation.id,1)); // on chage un vélo dans le camion
                    }*/
                ...
                // ORDRE DE MOUVEMENT
                this._movingTruckId.push(truck.id); // ajout du camion à la liste des camions en déplacement // DEBUT D'UN DEPLACEMENT
                // ajout d'un ordre de déplacement
                if(truck.currentStation.id == context.stations[19].id)
                    result.push(new MoveOrder(truck.id,context.stations[19].id)); 
                else
                    result.push(new MoveOrder(truck.id,context.stations[19].id));
