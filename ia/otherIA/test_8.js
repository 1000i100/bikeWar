	...
	,messageHandler: function(event) {
		if(event.data != null) {
			...
			try {
				...
				msg = Std.string(Std.string(e) + " " + Std.string(e.stack));
			}
...
MyIA.prototype = $extend(WorkerIA.prototype,{
	getOrders: function(context) {
		...
		while(_g1 < _g) {
			...
			if(truck.owner.id == this.id && truck.currentStation != null) {
				if(this._defenceTruckId == -1) this._defenceTruckId = truck.id;
				if(Lambda.has(this._movingTruckId,truck.id)) {
					...
					if(truck.currentStation.bikeNum < truck.currentStation.slotNum / 4 && truck.bikeNum > 0) {
						this.debugMessage = "pas assez";
						result.push(new com.tamina.bikewar.data.UnLoadingOrder(truck.id,truck.currentStation.id,truck.bikeNum));
					} else if(truck.currentStation.bikeNum > truck.currentStation.slotNum / 4 * 3) {
						this.debugMessage = "trop de velo";
						if(truck.bikeNum <= 5) result.push(new com.tamina.bikewar.data.LoadingOrder(truck.id,truck.currentStation.id,5)); else if(truck.bikeNum < com.tamina.bikewar.game.Game.TRUCK_NUM_SLOT) result.push(new com.tamina.bikewar.data.LoadingOrder(truck.id,truck.currentStation.id,1));
					} else {
						this.debugMessage = "test : " + truck.currentStation.bikeNum + " // " + truck.currentStation.slotNum;
						if(com.tamina.bikewar.game.GameUtils.hasStationEnoughBike(truck.currentStation)) result.push(new com.tamina.bikewar.data.LoadingOrder(truck.id,truck.currentStation.id,0));
					}
				} else {
					var sickBadStation = this.getSickBikeStations();
					var goodStations = this.getGoodBikeStations();
					var few = this.getFewBikeStations();
					var much = this.getMuchBikeStations();
					if(truck.bikeNum >= 5 && few.length > 0) {
						this._movingTruckId.push(truck.id);
						result.push(new com.tamina.bikewar.data.MoveOrder(truck.id,this.getNearestStations(truck,few).id));
					} else if(truck.bikeNum < 5 && much.length > 0) {
						this._movingTruckId.push(truck.id);
						result.push(new com.tamina.bikewar.data.MoveOrder(truck.id,this.getNearestStations(truck,much).id));
					} else if(goodStations.length > 0) {
						this._movingTruckId.push(truck.id);
						result.push(new com.tamina.bikewar.data.MoveOrder(truck.id,this.getNearestStations(truck,this.getOthersBikeStations()).id));
					}
				}
			} else {
			}
	...
	,getNearestStations: function(origin,stations) {
		var result = stations[0];
		var _g1 = 1;
		var _g = stations.length;
		while(_g1 < _g) {
			var i = _g1++;
			if((origin.currentStation == null || stations[i].id != origin.currentStation.id) && com.tamina.bikewar.game.GameUtils.getTravelDuration(origin,stations[i]) < com.tamina.bikewar.game.GameUtils.getTravelDuration(origin,result)) result = stations[i];
		}
		return result;
	}
	,getSickBikeStations: function() {
		var result = new Array();
		var _g1 = 0;
		var _g = this._currentContext.stations.length;
		while(_g1 < _g) {
			var i = _g1++;
			if(!com.tamina.bikewar.game.GameUtils.hasStationEnoughBike(this._currentContext.stations[i])) result.push(this._currentContext.stations[i]);
		}
		return result;
	}
	,getGoodBikeStations: function() {
		var result = new Array();
		var _g1 = 0;
		var _g = this._currentContext.stations.length;
		while(_g1 < _g) {
			var i = _g1++;
			if((this._currentContext.stations[i].owner == null || this._currentContext.stations[i].owner.id != this.id) && com.tamina.bikewar.game.GameUtils.hasStationEnoughBike(this._currentContext.stations[i])) result.push(this._currentContext.stations[i]);
		}
		return result;
	}
	,getFewBikeStations: function() {
		var result = new Array();
		var _g1 = 0;
		var _g = this._currentContext.stations.length;
		while(_g1 < _g) {
			var i = _g1++;
			if(!com.tamina.bikewar.game.GameUtils.hasStationEnoughBike(this._currentContext.stations[i]) && this._currentContext.stations[i].bikeNum < 9) result.push(this._currentContext.stations[i]);
		}
		return result;
	}
	,getOthersBikeStations: function() {
		var result = new Array();
		var _g1 = 0;
		var _g = this._currentContext.stations.length;
		while(_g1 < _g) {
			var i = _g1++;
			if(this._currentContext.stations[i].owner == null || this._currentContext.stations[i].owner.id != this.id) result.push(this._currentContext.stations[i]);
		}
		return result;
	}
	,getMuchBikeStations: function() {
		var result = new Array();
		var _g1 = 0;
		var _g = this._currentContext.stations.length;
		while(_g1 < _g) {
			var i = _g1++;
			if(!com.tamina.bikewar.game.GameUtils.hasStationEnoughBike(this._currentContext.stations[i]) && this._currentContext.stations[i].bikeNum > 12) result.push(this._currentContext.stations[i]);
		}
		return result;
	}
});
...
com.tamina.bikewar.core.PathFinder.prototype = {
	...
	,checkPath: function(target) {
		...
		if(currentJunction.links == null) console.log("Null Pointer Exception : links on Junctions " + currentJunction.id);
		var _g1 = 0;
