...
var MyIA = function() {
	this.executingMoveOrders = new Array();
	this._turnNum = 1;
	WorkerIA.call(this);
};
MyIA.__name__ = true;
...
MyIA.prototype = $extend(WorkerIA.prototype,{
	getOrders: function(context) {
		if(this._turnNum == 1) this.ennemyId = this.getEnnemyId(context);
		var result = new Array();
		var _g1 = 0;
		...
		while(_g1 < _g) {
			...
			if(truck[0].owner.id == this.id && truck[0].currentStation != null) {
				if(Lambda.find(this.executingMoveOrders,(function(truck) {
					return function(order) {
						return order.truckId == truck[0].id;
					};
				})(truck)) != null) {
					this.executingMoveOrders = this.executingMoveOrders.filter((function(truck) {
						return function(order1) {
							return order1.truckId != truck[0].id;
						};
					})(truck));
					if(this.isStationWellManaged(truck[0].currentStation)) {
						if(this.shouldLoad(truck[0])) result.push(new com.tamina.bikewar.data.LoadingOrder(truck[0].id,truck[0].currentStation.id,1)); else if(this.canUnload(truck[0])) result.push(new com.tamina.bikewar.data.UnLoadingOrder(truck[0].id,truck[0].currentStation.id,1)); else if(this.canLoad(truck[0])) result.push(new com.tamina.bikewar.data.LoadingOrder(truck[0].id,truck[0].currentStation.id,1));
					} else if(truck[0].currentStation.bikeNum < truck[0].currentStation.slotNum / 2) {
						var nbBikesToUnload = this.getNbBikeToUnload(truck[0]);
						if(nbBikesToUnload > 0) result.push(new com.tamina.bikewar.data.UnLoadingOrder(truck[0].id,truck[0].currentStation.id,nbBikesToUnload));
					} else if(truck[0].currentStation.bikeNum > truck[0].currentStation.slotNum / 2) {
						var nbBikesToLoad = this.getNbBikeToLoad(truck[0]);
						if(nbBikesToLoad > 0) result.push(new com.tamina.bikewar.data.LoadingOrder(truck[0].id,truck[0].currentStation.id,nbBikesToLoad));
					} else if(truck[0].bikeNum < com.tamina.bikewar.game.Game.TRUCK_NUM_SLOT / 2) result.push(new com.tamina.bikewar.data.LoadingOrder(truck[0].id,truck[0].currentStation.id,1)); else result.push(new com.tamina.bikewar.data.UnLoadingOrder(truck[0].id,truck[0].currentStation.id,1));
				} else {
					var stationToGo = this.chooseBestStation(context,truck[0]);
					if(stationToGo != null) {
						result.push(new com.tamina.bikewar.data.MoveOrder(truck[0].id,stationToGo.id));
						this.executingMoveOrders.push(new com.tamina.bikewar.data.MoveOrder(truck[0].id,stationToGo.id));
					}
				}
			} else {
			}
	...
	,chooseBestStation: function(context,truck) {
		var _g = this;
		var bestStation = this.findWithPredicate(context,truck,function(bs) {
			return !_g.isNotOwned(bs) && bs.owner.id == _g.ennemyId && _g.isStationWellManaged(bs);
		});
		if(bestStation == null) {
			bestStation = this.findWithPredicate(context,truck,function(bs1) {
				return !_g.isNotOwned(bs1) && bs1.owner.id == _g.id && _g.canRefillStation(bs1,truck);
			});
			if(bestStation == null) bestStation = this.findWithPredicate(context,truck,function(bs2) {
				return _g.isNotOwned(bs2) && _g.isStationWellManaged(bs2);
			});
		}
		return bestStation;
	}
	,aTruckIsGoingTo: function(context,station) {
		var res = Lambda.find(this.executingMoveOrders,function(move) {
			return move.targetStationId == station.id;
		});
		return res != null;
	}
	,findWithPredicate: function(context,truck,f) {
		var best = null;
		var distance = 0;
		var _g1 = 0;
		var _g = context.stations.length;
		while(_g1 < _g) {
			var i = _g1++;
			var bs = context.stations[i];
			if(this.aTruckIsGoingTo(context,bs)) continue;
			if(f(bs)) {
				if(best == null) {
					best = bs;
					distance = com.tamina.bikewar.game.GameUtils.getTravelDuration(truck.currentStation,best,context);
				} else {
					var bsDistance = com.tamina.bikewar.game.GameUtils.getTravelDuration(truck.currentStation,bs,context);
					if(bsDistance < distance) {
						best = bs;
						distance = bsDistance;
					}
				}
	...
	,canRefillStation: function(station,truck) {
		return !com.tamina.bikewar.game.GameUtils.hasStationEnoughBike(station) && (station.bikeNum < station.slotNum / 4 && station.slotNum / 4 - station.bikeNum < truck.bikeNum || station.bikeNum > station.slotNum * 3 / 4 && station.bikeNum - station.slotNum * 3 / 4 < com.tamina.bikewar.game.Game.TRUCK_NUM_SLOT - truck.bikeNum);
	}
	,getNbBikeToLoad: function(truck) {
		var nbShoudLoad = truck.currentStation.bikeNum - (truck.currentStation.slotNum / 2 | 0);
		var nbCouldLoad = com.tamina.bikewar.game.Game.TRUCK_NUM_SLOT - truck.bikeNum;
		if(nbCouldLoad > nbShoudLoad) return nbShoudLoad; else return nbCouldLoad;
	}
	,getNbBikeToUnload: function(truck) {
		var nbShoudUnload = (truck.currentStation.slotNum / 2 | 0) - truck.currentStation.bikeNum;
		var nbCouldUnload = truck.bikeNum;
		if(nbCouldUnload > nbShoudUnload) return nbShoudUnload; else return nbCouldUnload;
	}
	,isStationWellManaged: function(station) {
		if(station.bikeNum < station.slotNum * 2 / 5) return false; else if(station.bikeNum > station.slotNum * 3 / 5) return false; else return true;
	}
	,shouldLoad: function(truck) {
		return truck.bikeNum < com.tamina.bikewar.game.Game.TRUCK_NUM_SLOT / 2;
	}
	,canUnload: function(truck) {
		return truck.bikeNum > 0;
	}
	,canLoad: function(truck) {
		return truck.bikeNum < com.tamina.bikewar.game.Game.TRUCK_NUM_SLOT;
	}
	,getEnnemyId: function(context) {
		var _g = this;
		var ennemy = Lambda.find(context.players,function(x) {
			return x.id != _g.id;
		});
		return ennemy.id;
	}
	,isNotOwned: function(station) {
...
com.tamina.bikewar.core.PathFinder.prototype = {
	getPath: function(fromStation,toStation,map) {
		console.log(fromStation.name + " to " + toStation.name);
		this._map = map;
